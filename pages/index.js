...
// import searchInputRef from "next/input/searchInputRef"
...
export default function Home() {
  ...
  async function randomSearch(event) {
    event.preventDefault();
    const randomTerm =  await fetch(``).then((response) => response.json());
    ...
    <div>
      ...
      <form className='flex flex-col items-center mt-40'>
        <Image
         ...
         />
        <div className='flex w-full mt-7 mx-auto max-w-[90%] border border-gray-200 hover:shadow-lg focus-within:shadow-lg px-5 py-3 rounded-full items-center sm:max-w-xl lg:max-w-2xl'>
            <SearchIcon className='h-5 text-gray-500 mr-3 '/>
            <input ref={searchInputRef} type="text" className='flex-grow focus:outline-none' />
           <MicrophoneIcon className='h-5'/>
        </div >
        <div className='flex flex-col sm:flex-row w-[50%] space-y-2 mt-8 sm:space-y-0 sm:space-x-4 justify-center '>
        <button onClick={search} className='btn hover:bg-red-100'>Powpa Search</button>
        <button onClick={randomSearch} className='btn hover:bg-red-100'>I&apos;m Feeling Lucky</button>
        </div>
      </form>
